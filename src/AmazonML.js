...
import { DetectDocumentTextCommand, TextractClient, AnalyzeDocumentCommand } from "@aws-sdk/client-textract";
...
export async function analyzeImageML(type, imageData) {
  ...
  try {
    if (type == "labels") {
      ...
      const query = new AnalyzeDocumentCommand(params);
      let response = await textractClient.send(query);
      const isHandwritingDetected = response.Blocks.some(block => block.BlockType === "LINE" && isHandwritingLikely(block));

      
      if (isHandwritingDetected){
        returnData = {
          //type: "warning",
...
function isHandwritingLikely(block) {
  return block.Confidence < 90;
}
